...
def save_user_data(user_id, username, phone, timestamp, action_type=None):
    try:
        ...
        user_data = {
            ...
            "phone": phone,
            "timestamp": timestamp,
            "action_type": action_type  # "hotel_selection" или None
        }
...
async def hotel_selection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.message.reply_text(
        "Для зв'язку з нашим фахівцем щодо підбору готелю, будь ласка, поділіться вашим номером телефону. "
        "Натисніть кнопку нижче:",
        reply_markup=reply_markup
    )
...
async def process_contact(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if action_type == "hotel_selection":
        message_text = "Дякуємо! Ваш номер телефону збережено. Зараз ви будете перенаправлені до нашого фахівця з підбору готелів."
        button_text = "👨‍💼 Перейти до фахівця"
    else:
...
async def back_to_agent(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Всегда переходим на i_am_agent согласно инструкции
...
async def send_agent_testimonial(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if index >= len(AGENT_TESTIMONIAL_VIDEO_IDS):
        await query.message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("🔙 Повернутись", callback_data="i_am_agent")
            ]])
        )
        return
...
async def more_info(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Создаем клавиатуру с кнопками 2.1 (добавляем кнопку социальных сетей и подбора отеля)
    keyboard = [
...
async def send_testimonial(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    elif index == 6:  # Седьмой отзыв (двойной)
        # Для первого видео из пары двойного отзыва - кнопок не будет
        keyboard = None
    elif index == 7:  # Восьмой отзыв (тройной)
        # Для первых двух видео тройного отзыва - кнопок не будет
        keyboard = None
    ...
    try:
        ...
        # Отправка 7-го отзыва (двойного)
            ...
            keyboard_final = [
                [InlineKeyboardButton("😊 Більше відгуків", callback_data="testimonial_7")],
                [InlineKeyboardButton("👨‍💼 Навчання", callback_data="more_info"),
                 InlineKeyboardButton("💰 Оплатити", callback_data="training_info")]
            ]
...
async def training_info(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Клавиатура с кнопками (добавлена кнопка подбора отеля)
    keyboard = [
...
async def full_course(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Клавиатура с кнопками (добавляем кнопку подбора отеля)
    keyboard = [
...
async def next_video(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if next_index == 1:  # Для второго видео добавляем дополнительный текст
        ...
        if next_index < len(COURSE_VIDEOS) - 1:  # Не последнее видео
            ...
            # Добавляем кнопки к последнему видео (включая подбор отеля)
            keyboard = [
                [InlineKeyboardButton("✅ Я пройшов / пройшла курс", callback_data="course_completed")],
                [InlineKeyboardButton("🎁 Ексклюзив для турагентів✨", callback_data="i_am_agent")],
                [InlineKeyboardButton("📱 Соціальні мережі", callback_data="social_media")],
...
async def pro_full_course(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Клавиатура с кнопками - добавляем дополнительные кнопки
    keyboard = [
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Создаем клавиатуру с опциями админ-панели (добавлена кнопка просмотра запросов на подбор отеля)
    keyboard = [
...
def main() -> None:
    ...
    application.add_handler(CallbackQueryHandler(back_to_agent, pattern="^i_am_agent$"))
