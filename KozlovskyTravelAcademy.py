...
async def request_contact(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    context.user_data["action_type"] = "request_contact"  # ИСПРАВЛЕНИЕ: Добавлено сохранение типа действия
...
async def process_contact(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for admin_id in [254322794, 609094231]:
        try:
...
async def agent_testimonials(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "agent_testimonials":
        # ИСПРАВЛЕНИЕ: Сначала проверяем флаги Pro курса, чтобы они имели приоритет
        ...
        elif context.user_data.get("return_to_full_course"):
...
async def back_to_source(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ИСПРАВЛЕНИЕ: Улучшенная логика выбора направления возврата
...
def main() -> None:
    ...
    # ИСПРАВЛЕНИЕ: Обновлен ConversationHandler для рассылки сообщений
    broadcast_handler = ConversationHandler(
        ...
        # Это важно! Без этого параметра обработчик может не работать правильно
        allow_reentry=True
    )
    ...
    # ИСПРАВЛЕНИЕ: Удалены дублирующие обработчики, которые вызывали двойную отправку
