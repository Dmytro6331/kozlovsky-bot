...
# Admin IDs - список ID администраторов (замените на реальные ID)
ADMIN_IDS = [609094231]  # Добавлен реальный ID администратора

# Файл для хранения данных пользователей
USERS_DATA_FILE = "interested_users.json"

# Для работы с состояниями разговора
PHONE, PROCESSING = range(2)

# Функция для сохранения данных пользователей в файл
def save_user_data(user_id, username, phone, timestamp):
    try:
...
async def process_contact(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    save_user_data(user.id, user.username, phone_number, timestamp)
    
    # Сообщаем пользователю о переходе
    await update.message.reply_text(
        "Дякуємо! Ваш номер телефону збережено. Зараз ви будете перенаправлені до нашого представника.",
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("👨‍💼 Перейти до представника", url=context.user_data.get("next_url", "https://t.me/kozlovsky_official2"))
        ]])
    )
...
# Определите словари с данными здесь
VIDEO_FILE_IDS = {
...
# Course circle video IDs
COURSE_CIRCLE_VIDEO_IDS = [
    "DQACAgIAAxkBAAIIVGfkfSw9DRCLhuYDlyaD_B2paQOgAAKLbAACt4kgSxAFrzCBlGhRNgQ",  # 1-й кружок
    "DQACAgIAAxkBAAIJkmflMKduFuZADf3SAAG3lTtyMeN4-wACm2wAAreJIEsSi8dcI9VS5DYE",  # 2-й кружок
    "DQACAgIAAxkBAAIJYWflI39WeiGlWVUT0HXyv7o8qTLBAAKcbAACt4kgSwu47maxutHqNgQ",  # 3-й кружок
    "DQACAgIAAxkBAAIJlWflMotlTylNMuJc-k3FP0zcN_V1AAKmbAACt4kgS9LRJlVLOomfNgQ",  # 4-й кружок
    "DQACAgIAAxkBAAIH92fkKwzpiOgD0sMKP8QwmE9bzalcAAKfbAACt4kgS73Aqchf7HvuNgQ"   # 5-й кружок
]
...
# Testimonial file IDs
TESTIMONIAL_FILE_IDS = [
    "BAACAgIAAxkBAAIHWGfimLuI5nveS9A6qIGZ_yUg9yNjAAJgbQACpkAISxOBJPtS2rasNgQ",  # 1-й отзыв
    "BAACAgIAAxkDAAIHKmfhRoxkHbET16Q2mgN4Rc0p8ZydAAIBigACSB3QSjYVrKeUJHeCNgQ",  # 2-й отзыв
    "DQACAgIAAxkDAAIHT2fhsDn-fiMKvScGiLhjgWxjrBr5AALZZQACQDLISqIU-Nfd0z42NgQ",  # 3-й отзыв
    "BAACAgIAAxkDAAIHO2fhWPNfRh-Q3SFovoTLQ1O_arrEAAKFbgAClDjgSspPuuTGWSK_NgQ",  # 4-й отзыв
    "DQACAgIAAxkDAAIHLmfhRrA8alrwMWc8eFediqUV_-dRAAKsXwACnszISqQcU06eoadsNgQ"   # 5-й отзыв
]

# Testimonial double file IDs
...
AGENT_TESTIMONIAL_VIDEO_IDS = [
    ...
    # Восьмой отзыв - одно видео
    ["BAACAgIAAxkBAAIRG2fqqkQChl8gWet5eL6KHu596iklAAL3dAACaMvQSi9eDEz2HiyXNgQ"],
    
    # Девятый отзыв - одно видео
    ["BAACAgIAAxkBAAIRHGfqqpPME4GZD3eV8gZqFqHV0H__AAKbdAACyzRZS7bQynFmlbb2NgQ"]
]
...
# Agent video IDs and descriptions
AGENT_VIDEO_IDS = [
    "DQACAgIAAxkBAAIH_GfkLfrlIbR0K_ohh7jZfhT8lYCVAAJ7ZAAC__cIS41AF8pr7dCnNgQ",  # 1-й кружок
    "DQACAgIAAxkBAAIJmGflM_5OWzi32UkjhLaF7YjlwLSHAAKDZAAC__cIS5gP4vSrYqYLNgQ",  # 2-й кружок
    "DQACAgIAAxkBAAIJmWflM_6PBymrXmh122qyUVmDJCIQAAKRZAAC__cIS3Rye3pphwABkDYE",  # 3-й кружок
    "DQACAgIAAxkBAAIJmmflM_71YDr49u84_KKlGBak8UiMAAKXZAAC__cIS07rF5wdaU6XNgQ",  # 4-й кружок
    "DQACAgIAAxkBAAIJnGflM_4LA8hThVMBeMNsdFZLHqwnAAJLZQAC__cISytJ4FdaBtuvNgQ",  # 5-й кружок
    "DQACAgIAAxkBAAIJm2flM_6ME9J-vwgla9m8IZQBNnUtAAIqZQAC__cISwLqWAkokY6TNgQ",  # 6-й кружок
    "DQACAgIAAxkBAAIJnWflM_7vKUx3_4UmTMutJFRLYWyxAAJaZQAC__cIS0RvcpZXvKvUNgQ",  # 7-й кружок
    "DQACAgIAAxkBAAIJnmflM_7j3AvErwjZQEYaL_bnU4cwAAJlZQAC__cIS48bt-SP_5SZNgQ",  # 8-й кружок
    "DQACAgIAAxkBAAIJn2flM_5rwGYvZHVRey3pBrCYDhbbAAJuZQAC__cISxA-_nK7KCGzNgQ",  # 9-й кружок
    "DQACAgIAAxkBAAIJoGflM_7RsoD5XX9h2TXHaoZwrQvGAAJzZQAC__cISwABT3xwkpajsjYE",  # 10-й кружок
    "DQACAgIAAxkBAAIJoWflM_5RWTnAXBiZ3UxRd3kmI-NgAAJ3ZQAC__cISzr6TJiSQ7m0NgQ"   # 11-й кружок
]
...
PRO_COURSE_VIDEOS = [
    ...
    "https://youtube.com/live/NynXPJwjHKM",
    "https://youtube.com/live/g23JEImPhIY"
]
...
async def social_media(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    keyboard = [
        [InlineKeyboardButton("Instagram", url="instagram.com/kozlovsky_official"),
         InlineKeyboardButton("TikTok", url="https://www.youtube.com/@Enjoy-World")],
...
# Функция для отображения наших продуктов (прямой переход по URL)
async def our_products(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle our products button press."""
    query = update.callback_query
    await query.answer(text="Переходимо на сайт з нашими продуктами", show_alert=False)
    
    # Используем URL вместо кнопок
    url = "https://datour.club/#partners"
    ...
    await query.message.reply_text(
        "🌟 Натисніть кнопку нижче, щоб перейти на сайт з нашими продуктами:",
        reply_markup=reply_markup
    )

# Функция для сохранения источника запроса для отзывов турагентов
async def save_source_for_return(update: Update, context: ContextTypes.DEFAULT_TYPE, callback_name: str) -> None:
    """Сохранить источник запроса для возврата после просмотра отзывов или договора."""
    source_info = {
        "callback_name": callback_name,
        "index": context.user_data.get("course_video_index", 0) if callback_name == "i_am_agent" else None
    }
    context.user_data["return_to_source"] = source_info

# Функция для отображения отзывов турагентов
async def agent_testimonials(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle agent testimonials button press."""
    query = update.callback_query
    await query.answer()
    
    # Сохраняем источник запроса для возврата
    await save_source_for_return(update, context, query.data)
...
# Функция для возврата к источнику запроса после просмотра отзывов или договора
async def back_to_agent(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Вернуться к источнику запроса после просмотра отзывов или договора."""
    query = update.callback_query
    await query.answer()
    
    source_info = context.user_data.get("return_to_source", {"callback_name": "i_am_agent", "index": 0})
    callback_name = source_info.get("callback_name", "i_am_agent")
    
    # Если источник - "Я турагент", восстанавливаем индекс видео и отправляем его снова
    if callback_name == "i_am_agent":
        context.user_data["course_video_index"] = source_info.get("index", 0)
        await i_am_agent(update, context)
    else:
        # В других случаях можно добавить специфичные действия или просто вернуться в начало
...
async def send_agent_testimonial(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if index >= len(AGENT_TESTIMONIAL_VIDEO_IDS):
        await query.message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("🔙 Повернутись", callback_data="back_to_agent")
            ]])
        )
        return
...
# Функция для отображения договора
async def contract(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    keyboard = [[InlineKeyboardButton("🔙 Повернутись", callback_data="back_to_agent")]]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE, is_callback=False) -> None:
    ...
    # Новый текст приветственного сообщения
    welcome_text = (
        "✨ Привіт, друзі! ✨\n"
...
async def more_info(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Создаем клавиатуру с кнопками 2.1 (добавляем кнопку социальных сетей)
    keyboard = [
...
# 3. Кнопка "Про що цей курс" (теперь содержит "Я турагент")
async def about_course(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Отправляем первое видео для агентов (без заголовка)
...
async def send_testimonial(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if index == 0:  # Первый отзыв
        keyboard = [
            [InlineKeyboardButton("😊 Більше відгуків", callback_data="testimonial_1")],
            [InlineKeyboardButton("👨‍💼 Навчання", callback_data="more_info"),
             InlineKeyboardButton("💰 Оплатити", callback_data="training_info")]
        ]
    elif index == 1:  # Второй отзыв
        keyboard = [
            [InlineKeyboardButton("😊 Більше відгуків", callback_data="testimonial_2")],
            [InlineKeyboardButton("👨‍💼 Навчання", callback_data="more_info"),
             InlineKeyboardButton("💰 Оплатити", callback_data="training_info")]
        ]
    elif index == 2:  # Третий отзыв
        keyboard = [
            [InlineKeyboardButton("😊 Більше відгуків", callback_data="testimonial_3")],
            [InlineKeyboardButton("👨‍💼 Навчання", callback_data="more_info"),
             InlineKeyboardButton("💰 Оплатити", callback_data="training_info")]
        ]
    elif index == 3:  # Четвертый отзыв
        keyboard = [
            [InlineKeyboardButton("😊 Більше відгуків", callback_data="testimonial_4")],
            [InlineKeyboardButton("👨‍💼 Навчання", callback_data="more_info"),
             InlineKeyboardButton("💰 Оплатити", callback_data="training_info")]
        ]
    elif index == 4:  # Пятый отзыв
        keyboard = [
            [InlineKeyboardButton("😊 Більше відгуків", callback_data="testimonial_5")],
            [InlineKeyboardButton("👨‍💼 Навчання", callback_data="more_info"),
             InlineKeyboardButton("💰 Оплатити", callback_data="training_info")]
        ]
    elif index == 5:  # Шестой отзыв (отдельный)
        keyboard = [
            [InlineKeyboardButton("😊 Більше відгуків", callback_data="testimonial_6")],
            [InlineKeyboardButton("👨‍💼 Навчання", callback_data="more_info"),
             InlineKeyboardButton("💰 Оплатити", callback_data="training_info")]
        ]
    elif index == 6:  # Седьмой отзыв (последний, двойной)
        # Для первого видео из пары двойного отзыва - кнопок не будет
        keyboard = None
    
    reply_markup = InlineKeyboardMarkup(keyboard) if keyboard else None
    
    try:
        # Отправка основных 5 отзывов
        if index < 5:
            await query.message.reply_video(
        ...
        # Отправка 7-го отзыва (двойного)
            ...
            reply_markup_final = InlineKeyboardMarkup(keyboard_final)
...
async def training_info(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Клавиатура с кнопками (добавлена кнопка социальных сетей)
    keyboard = [
...
async def payment_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Правка: Изменен текст сообщения об успешной оплате
    await update.message.reply_text(
...
async def full_course(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Клавиатура с кнопками (добавляем кнопку социальных сетей)
    keyboard = [
...
async def next_video(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if next_index < len(COURSE_VIDEOS) - 1:  # Не последнее видео
        ...
        # Добавляем кнопки к последнему видео (включая социальные сети)
        keyboard = [
            [InlineKeyboardButton("✅ Я пройшов / пройшла курс", callback_data="course_completed")],
            [InlineKeyboardButton("🎁 Ексклюзив для турагентів✨", callback_data="i_am_agent")],
            [InlineKeyboardButton("📱 Соціальні мережі", callback_data="social_media")]
...
async def pro_full_course(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Клавиатура с кнопками - оставляем только кнопку "Цікавить співпраця"
    keyboard = [
...
# 14. "🎁 Ексклюзив для турагентів✨" (бывшая "Я турагент")
async def i_am_agent(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Отправляем первое видео (без заголовка)
...
# Админ-панель - отображение данных пользователей
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    keyboard = [
        [InlineKeyboardButton("📊 Показати всіх користувачів", callback_data="admin_show_all")],
...
# Обработка текстовых сообщений для поиска упоминаний оплаты
...
def main() -> None:
    ...
    conv_handler = ConversationHandler(
        entry_points=[CallbackQueryHandler(request_contact, pattern="^request_contact$")],
        states={
    ...
    application.add_handler(CallbackQueryHandler(back_to_agent, pattern="^back_to_agent$"))
