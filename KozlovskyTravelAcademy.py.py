...
async def process_contact(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Forward the phone number to admins (ID: 6540912032 and 609094231)
    admin_message = ""
    if action_type == "hotel_selection":
    ...
    for admin_id in [6540912032, 609094231]:
        try:
...
async def agent_testimonials(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "agent_testimonials":
        # Determine transition source from user context
...
# 22. User tracking modifications
# Functions request_contact and hotel_selection are already implemented above with these requirements
...
async def broadcast_photo_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Send photo with confirmation buttons
...
async def broadcast_video_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Send video with confirmation buttons
    await update.message.reply_video(
        video=video.file_id,
...
async def broadcast_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Warn user about broadcast start
    await query.message.edit_text(
...
async def broadcast_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keys_to_remove = [
        ...
        "broadcast_cancelled", "creating_broadcast"
    ]
...
def main() -> None:
    ...
    # Add ConversationHandler for message broadcast
    broadcast_handler = ConversationHandler(
        ...
        # This is important! Without this parameter, the handler may not work correctly
        allow_reentry=True
    )
    ...
    # Handlers for callback_data related to broadcast
    application.add_handler(CallbackQueryHandler(broadcast_target_selected, pattern="^target_"), group=5)
    application.add_handler(CallbackQueryHandler(broadcast_cancel, pattern="^broadcast_cancel$"), group=5)
    application.add_handler(CallbackQueryHandler(broadcast_content_type_selected, pattern="^content_type_"), group=5)
    
    # IMPORTANT: Add additional handler for continue_broadcast outside the ConversationHandler
    # This ensures the callback is properly processed even if there are issues with the ConversationHandler state
    application.add_handler(CallbackQueryHandler(continue_broadcast, pattern="^continue_broadcast$"), group=5)
    application.add_handler(CallbackQueryHandler(broadcast_cancel_confirmation, pattern="^broadcast_cancel_confirm$"), group=5)
